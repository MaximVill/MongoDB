/**
 * server.js — Football Stats DB  (Node.js + Express + MongoDB)
 * ─────────────────────────────────────────────────────────────
 * Архитектура:
 *   1. При старте проверяет, пусты ли коллекции.
 *      Если да — читает players.json / matches.json / users.json
 *      из той же папки и импортирует данные в MongoDB.
 *   2. Все операции UI (поиск, добавление матча, бан) идут через
 *      REST API → MongoDB.  Данные реально сохраняются в БД.
 *   3. Фронтенд раздаётся из папки public/ как статика.
 *
 * Запуск:
 *   npm install
 *   node server.js
...
async function seedCollection(colName, filename) {
  ...
  if (count > 0) {
    console.log(`ℹ️   ${colName}: уже содержит ${count} документов, пропускаем импорт`);
    return;
  }
  ...
  console.log(`📥  ${colName}: импортировано ${docs.length} документов из ${filename}`);
}
...
app.get('/api/overview', wrap(async (req, res) => {
  ...
  const agg = await db.collection('players')
...
// ══════════════════════════════════════════════════════════════
//  PLAYERS — маршруты
//  ВАЖНО: специфичные маршруты объявляем ДО /:name
// ══════════════════════════════════════════════════════════════

// Топ-N бомбардиров (GET /api/players/top?limit=5)
app.get('/api/players/top', wrap(async (req, res) => {
  const limit = parseInt(req.query.limit) || 5;
  const list  = await db.collection('players')
    .find({}, { projection: {
      name: 1, team: 1, nationality: 1, position: 1,
      jerseyNumber: 1, 'stats.goals': 1, 'stats.assists': 1
    }})
    .sort({ 'stats.goals': -1 })
    .limit(limit)
    .toArray();
...
app.get('/api/players/search', wrap(async (req, res) => {
...
app.get('/api/players/stats/by-team', wrap(async (req, res) => {
  const list = await db.collection('players').aggregate([
    {
      $group: {
        _id: '$team',
...
app.put('/api/players/:name/goal', wrap(async (req, res) => {
  const name   = decodeURIComponent(req.params.name);
  const result = await db.collection('players').updateOne(
    { name },
    { $inc: { 'stats.goals': 1 } }
  );
  if (!result.matchedCount) return res.status(404).json({ error: 'Игрок не найден' });
  res.json({ ok: true });
}));

// Заблокировать комментарий у игрока
app.put('/api/players/:name/ban-comment', wrap(async (req, res) => {
  const name     = decodeURIComponent(req.params.name);
  ...
  if (!result.matchedCount) return res.status(404).json({ error: 'Комментарий не найден' });
  res.json({ ok: true });
}));
...
app.get('/api/matches/stats/goals-by-team', wrap(async (req, res) => {
  const list = await db.collection('matches').aggregate([
    { $unwind: '$events' },
...
app.get('/api/matches', wrap(async (req, res) => {
  ...
  const list   = await db.collection('matches')
    .find(filter)
    .sort({ date: -1 })
    .toArray();
  res.json(list);
...
app.post('/api/matches', wrap(async (req, res) => {
  const {
    homeTeam, awayTeam, date, tournament,
    season, stadium, attendance, scoreHome, scoreAway
  } = req.body;
...
app.put('/api/matches/:id/ban-comment', wrap(async (req, res) => {
  const { author } = req.body;
  if (!author) return res.status(400).json({ error: 'Укажите автора' });

  let _id;
  try { _id = new ObjectId(req.params.id); }
  catch { return res.status(400).json({ error: 'Неверный ID матча' }); }
...
app.put('/api/users/:username/ban', wrap(async (req, res) => {
